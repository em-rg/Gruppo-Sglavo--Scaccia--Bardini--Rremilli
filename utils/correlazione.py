...
def analizza_correlazioni_g3(df, correlation_matrix):
...
def grafico_top_correlazioni_g3(df, correlation_matrix):
    g3_correlations = correlation_matrix['G3'].abs().sort_values(ascending=False)
...
def main():
    ...
    analizza_voti(df)
    analizza_demografia(df)
    correlation_matrix = matrice_correlazione(df)
    analizza_correlazioni_g3(df, correlation_matrix)
    grafico_top_correlazioni_g3(df, correlation_matrix)
    scatter_voti(df)
...
plt.tight_layout()
plt.show()
#G1, G2 e G3 hanno una forte correlazione, gli studenti hanno prestazioni costanti
#c'è una correlazione positiva tra higher e G3: l'aspirazione all'istruzione superiore migliora le performance
#con Medu e Fedu vediamo che l'istruzione superiore influenza positivamente i risultati scolastici
#famrel: buone relazioni familiari correlano con migliori voti
#Dalc/Walc: consumo di alcol correlato con voti più bassi
